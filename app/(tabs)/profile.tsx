...
export default function ProfileScreen() {
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <SettingRow label={t('profile.theme')} value={theme === 'dark' ? t('profile.dark') : t('profile.light')} onPress={() => setTheme(theme === 'dark' ? 'light' : 'dark')} />
      <SettingRow label={t('profile.language')} value={lang === 'en' ? 'English' : '繁體中文'} onPress={() => setLang(lang === 'en' ? 'zh-TW' : 'en')} />
      <SettingRow label={t('profile.sound')} value={sound ? 'ON' : 'OFF'} onPress={() => setSound(!sound)} />
