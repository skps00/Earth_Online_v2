  ...
  async getToday(lang: Language): Promise<QuestDisplay[]> {
    ...
    return db.getAllAsync<QuestDisplay>(
      `SELECT dq.*, tt.value as title, td.value as description,
       ...
       LEFT JOIN translations tt ON dq.id = tt.entity_id AND tt.lang = ? AND tt.field = 'title'
       LEFT JOIN translations td ON dq.id = td.entity_id AND td.lang = ? AND td.field = 'description'
